...
export default function Weather({ weather }) {
  ...
  return (
    <div
      ...
      style={{ zIndex: tasks.indexOf("weather") + 1 }}
    >
      <input
        type="text"
        placeholder="Search..."
        onKeyDown={async (e) => {
          if (e.code == "Enter" || e.code == "enter") {
            const weather = await fetch(
              `http://api.openweathermap.org/data/2.5/weather?q=${e.target.value}&units=metric&APPID=c3c4f72debc080219009dd186a5092cc`
            );
            const res = await weather.json();
            if (res.message !== "city not found") setData(res);
